...
cardapio.metodos = {
    ...
    obterItensCardapio: (categoria = 'mods', vermais = false) => {
    ...
    carregarValores: () => {
        ...
        $.each(MEU_CARRINHO, (i, e) => {
            ...
            if ((i + 1) == MEU_CARRINHO.length) {
                ...
                $("#lblValorEntrega").text(`+ 5,5%`);
                $("#lblValorTotal").text(`R$ ${((VALOR_CARRINHO * VALOR_ENTREGA)+ VALOR_CARRINHO).toFixed(2).replace('.', ',')}`);
            }
    ...
    resumoPedido: () => {
        ...
        if (cpf.length <= 0) {
            cardapio.metodos.mensagem('Informe o CPF, por favor.');
            $("#txtCPF").focus();
            return;
        }
